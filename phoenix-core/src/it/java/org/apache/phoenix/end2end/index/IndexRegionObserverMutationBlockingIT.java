...
public class IndexRegionObserverMutationBlockingIT extends BaseTest {
  ...
  public void testMutationBlockedOnDataTableWithIndex() throws Exception {
    ...
    try (Connection conn = DriverManager.getConnection(getUrl())) {
      ...
      ClusterRoleRecord blockingCrr =
        new ClusterRoleRecord("failover_test", HighAvailabilityPolicy.FAILOVER, haAdmin.getZkUrl(),
          ClusterRoleRecord.ClusterRole.ACTIVE_TO_STANDBY, "standby-zk-url",
  ...
  public void testMutationAllowedWhenNotBlocked() throws Exception {
    ...
    try (Connection conn = DriverManager.getConnection(getUrl())) {
      ...
      ClusterRoleRecord nonBlockingCrr = new ClusterRoleRecord("active_test",
        ...
        "standby-zk-url", ClusterRoleRecord.ClusterRole.STANDBY, 1L);
  ...
  public void testMutationBlockingTransition() throws Exception {
    ...
    try (Connection conn = DriverManager.getConnection(getUrl())) {
      ...
      crr = new ClusterRoleRecord("transition_test", HighAvailabilityPolicy.FAILOVER,
        haAdmin.getZkUrl(), ClusterRoleRecord.ClusterRole.ACTIVE_TO_STANDBY, "standby-zk-url",
      ...
      crr = new ClusterRoleRecord("transition_test", HighAvailabilityPolicy.FAILOVER,
        haAdmin.getZkUrl(), ClusterRoleRecord.ClusterRole.ACTIVE, "standby-zk-url",
        ClusterRoleRecord.ClusterRole.ACTIVE_TO_STANDBY, 3L);
      haAdmin.createOrUpdateDataOnZookeeper(crr);
      Thread.sleep(ZK_CURATOR_EVENT_PROPAGATION_TIMEOUT_MS);
