...
public class PhoenixServerRpcIT extends BaseTest {
  ...
  public void testIndexQos() throws Exception {
    ...
    try {
      ...
      // An index row is updated twice, once before the data table row, and once after it. Thus, the
      // factory is invoked twice
      ...
      // drop index table
      conn.createStatement().execute("DROP INDEX " + indexName + " ON " + dataTableFullName);
      ...
      Mockito.verify(TestPhoenixIndexRpcSchedulerFactory.getIndexRpcExecutor(), never())
        .dispatch(Mockito.any(CallRunner.class));
  ...
  private void upsertSelectRows(Connection conn, String tableName1, String tableName2)
    throws SQLException {
    PreparedStatement stmt = conn.prepareStatement(
      "UPSERT INTO " + tableName2 + " (k, v1, v2) SELECT k, v1, v2 FROM " + tableName1);
    stmt.execute();
    conn.commit();
  }

  /**
   * Verifies that the given tables each have a single region and are on different region servers.
   * If they are on the same server moves tableName2 to the other region server.
   */
  private void ensureTablesOnDifferentRegionServers(String tableName1, String tableName2)
    throws Exception {
    ...
    // if data table and index table are on same region server, move the index table to the other
    // region server
    ...
    assertNotEquals(
      "Tables " + tableName1 + " and " + tableName2 + " should be on different region servers",
