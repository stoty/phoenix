...
public class CreateIndexCompiler {
  ...
  private String getValue(PDataType type) {
    if (type instanceof PNumericType) {
      ...
      type instanceof PDate || type instanceof PUnsignedDate || type instanceof PTime
      ...
      type instanceof PDateArray || type instanceof PUnsignedDateArray || type instanceof PTimeArray
        || type instanceof PUnsignedTimeArray || type instanceof PTimestampArray
        || type instanceof PUnsignedTimestampArray
    ) {
  ...
  /**
   * Verifies that index WHERE clause does not include a subquery and it can be evaluated on a
   * single data table row.
   */
  private void verifyIndexWhere(ParseNode indexWhere, StatementContext context,
    ...
    if (indexWhereParseNodeVisitor.hasSubquery) {
      throw new SQLExceptionInfo.Builder(SQLExceptionCode.INVALID_INDEX_WHERE_WITH_SUBQUERY).build()
        .buildException();
    }
    ...
    // if table name has lowercase characters, query would provide escaped table name
    // so we need to unescape it to be able to lookup the ptable in the cache.
    PTable dataTable =
      connection.getTable(SchemaUtil.getUnEscapedFullName(dataTableName.toString()));
    ...
    try (
      ...
      while (dataTableNameAndMutationIterator.hasNext()) {
        ...
        if (
          java.util.Arrays.equals(dataTableNameAndMutation.getFirst(),
      ...
      if (!indexWhereExpression.evaluate(tuple, ptr)) {
        throw new SQLExceptionInfo.Builder(SQLExceptionCode.CANNOT_EVALUATE_INDEX_WHERE).build()
          .buildException();
      }
    } finally {
      ...
      if (list != null) {
        for (Pair<String, Object> pair : list) {
          if (pair.getFirst().equals(PhoenixDatabaseMetaData.SALT_BUCKETS)) {
            throw new SQLExceptionInfo.Builder(SQLExceptionCode.CANNOT_SALT_LOCAL_INDEX).build()
              .buildException();
          }
        }
      }
  ...
  /**
   * Helper method to validate CREATE INDEX statements on SYSTEM tables. 1. Pass if scheme name not
   * provided, assumption is - it not a SYSTEM table. 2. Fail if SYSTEM_CATALOG_INDEXES_ENABLED not
   * enabled 3. Fail if table other than SYSTEM.CATALOG
   */
  private void verifyDataTable(PhoenixConnection connection, NamedTableNode table)
    throws SQLException {
    ...
    if (
      ...
      throw new SQLExceptionInfo.Builder(SQLExceptionCode.SYSTEM_TABLE_INDEXES_NOT_ENABLED).build()
        .buildException();
    }
    ...
    if (
      ...
      throw new SQLExceptionInfo.Builder(SQLExceptionCode.CANNOT_INDEX_SYSTEM_TABLE).build()
        .buildException();
    }
  }
}
